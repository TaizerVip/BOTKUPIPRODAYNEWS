...
def init_db():
    ...
    c.execute(
        "INSERT OR IGNORE INTO bot_settings (key, value) VALUES ('maintenance_message', '🔧 Бот временно недоступен. Ведутся технические работы.')")
...
def get_bot_settings_keyboard():
    ...
    keyboard = [
        ...
        [InlineKeyboardButton(f"⏱️ КД на посты: {cooldown} мин [{cooldown_status}]",
                              callback_data="admin_edit_cooldown")],
    ...
    if chat.type == "private":
        ...
        if text == "📋 Отправить объявление":
            ...
            try:
                ...
                await update.message.forward(GROUP_CHAT_ID)
